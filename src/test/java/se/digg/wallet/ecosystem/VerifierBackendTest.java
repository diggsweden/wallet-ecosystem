...
class VerifierBackendTest {
  ...
  void acceptsValidSdJwtVcCredential() throws Exception {
    ...
    verifierBackend.tryValidateSdJwtVc(vpToken, nonce)
        .then()
  ...
  void rejectsUntrustedPidIssuer() throws Exception {
    ...
    // 3. Validate SD-JWT VC using the utility endpoint
    // Note: We check that the list of errors contains the expected trust error.
    // The backend might return additional follow-up errors.
    verifierBackend.tryValidateSdJwtVc(vpToken, nonce)
        .then()
        .assertThat().statusCode(400)
        .and().body("error", hasItem("IssuerCertificateIsNotTrusted"));
