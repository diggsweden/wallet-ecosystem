...
class KeycloakTest {
  ...
  @ParameterizedTest
   @ValueSource(strings = {"realms/master/console/", "realms/admin/master/console"})
  void masterRealmAndAdminConsoleIsBlockedExternally() {
    given()
        .when().get(KEYCLOAK.getResourceRoot().resolve())
        .then().assertThat().statusCode(anyOf(is(403), is(404)));
  }
}
