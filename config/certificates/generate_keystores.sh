...
# --- Wallet Client Gateway Note ---
# NOTE: The Wallet Client Gateway keystore (keystore-wallet-app-bff-local.p12) is NOT
# generated by this script. It must be copied from the 'wallet-client-gateway'
# repository (docker-compose/config/keystore-wallet-app-bff-local.p12).
#
# WHY: The gateway's public key is hard-coded in the Keycloak realm export of that
# project. Regenerating it here would break the OIDC authentication flow.
