...
class KeycloakTest {
  ...
  void pidIssuerRealmLoadsResourcesExternally(String path) {
    ...
    Jsoup.parse(html, KEYCLOAK.toString())
        .select("[href*=/resources/], [src*=/resources/]")
        .forEach(el -> {
          String ref = el.hasAttr("href") ? el.absUrl("href") : el.absUrl("src");
          given().when().get(ref).then().assertThat().statusCode(200);
        });
  }
